...
function atualizarPainelOperacional() {
    ...
    const viagensDoMes = fullHistoricoDataOp.filter(d => {
        ...
        const transp = String(d.transportadora || "").toUpperCase();
        if (!transp.includes('SERRANA')) return false;
    ...
    const elMetaVMes = document.getElementById('disp_v_meta_mes');
    const elPercVMes = document.getElementById('disp_v_perc_mes');
    if(elMetaVMes) elMetaVMes.innerText = metaViagensMes.toLocaleString('pt-PT');
    if(elPercVMes) {
        const percV = metaViagensMes > 0 ? (totalViagensMesRealizado / metaViagensMes) * 100 : 0;
        elPercVMes.innerText = `${percV.toFixed(1)}%`;
    }

    const elMetaVolMes = document.getElementById('disp_vol_meta_mes');
    const elPercVolMes = document.getElementById('disp_vol_perc_mes');
    if(elMetaVolMes) elMetaVolMes.innerText = metaVolumeMes.toLocaleString('pt-PT') + ' m³';
    if(elPercVolMes) {
...
function renderLeaderboards(data, diasConsiderados = 1) {
    ...
    if(bVol) {
        ...
        if (globalTopVol.length === 0) {
            bVol.innerHTML = `<tr><td colspan="6" class="text-center py-4 text-slate-500">Nenhum veículo da Serrana encontrado no período.</td></tr>`;
        }
    }
    ...
    if(bCiclo) {
        ...
        if (globalTopCiclo.length === 0) {
            bCiclo.innerHTML = `<tr><td colspan="5" class="text-center py-4 text-slate-500">Nenhum ciclo da Serrana encontrado no período.</td></tr>`;
        }
    }
}
...
function renderDashboardsGerenciais(data) {
    ...
    if(tRes) {
        const dSerrana = data.filter(d=>String(d.transportadora||'').toUpperCase().includes('SERRANA'));
        const dOutras = data.filter(d=>!String(d.transportadora||'').toUpperCase().includes('SERRANA'));
        ...
        tRes.innerHTML = `
            <tr class="hover:bg-slate-800/30 transition-colors border-b border-slate-700/50">
                <td class="px-4 py-3 font-bold text-white">Serrana</td>
...
function renderIndicadoresExtras(data) {
    ...
    if (tbodyCaixa) {
        ...
        if (caixaArr.length === 0) {
            tbodyCaixa.innerHTML = `<tr><td colspan="3" class="text-center py-4 text-slate-500">Nenhum dado encontrado.</td></tr>`;
        } else {
            caixaArr.forEach(x => {
                const tr = `<tr>
    ...
    if (tbodyPiores) {
        ...
        if (pioresCiclos.length === 0) {
            tbodyPiores.innerHTML = `<tr><td colspan="4" class="text-center py-4 text-slate-500">Nenhum ciclo encontrado.</td></tr>`;
        } else {
            pioresCiclos.forEach(x => {
                ...
                const tr = `<tr>
                    ...
                    <td class="px-4 py-3 text-right font-mono text-rose-400 font-bold" title="Ciclo real do veículo. Para a média geral, foi considerado o valor normalizado.">${valorExibido}</td>
                </tr>`;
...
window.exportarTopVolume = function() {
    ...
    const wsData = [
        ['Posição', 'Placa', 'Transportadora', 'Total Viagens', 'Volume Total (m³)', 'Média Viagens/Dia', 'Cumpriu Meta (≥ 2/dia)?']
    ];
    
    globalTopVol.forEach((x, i) => {
        const viagensPorDia = diasConsideradosGlobais > 0 ? (x.v / diasConsideradosGlobais) : x.v;
        const cumpriuMeta = viagensPorDia >= 2 ? 'SIM' : 'NÃO';
