console.log("Módulo de Absenteísmo carregado com sucesso!");
...
window.buscarDescricaoCidAbs = function() {
    ...
    if (!descricao) {
        ...
        const categorias = {
            'A': 'Doenças Infecciosas e Parasitárias', 'B': 'Doenças Infecciosas / Virais',
        ...
        if (categorias[letra]) descricao = categorias[letra] + ' (Especifique o motivo abaixo)';
    }
...
window.mudarAbaAbsenteismo = function(aba) {
    ...
    const abas = ['todos', 'ATESTADO', 'FALTA', 'BANCO_HORAS'];
    abas.forEach(a => {
...
window.renderizarTabelaAbsenteismo = function() {
    ...
    if (window.abaAtualAbsenteismo !== 'todos') {
        lista = lista.filter(item => item.tipo_registro === window.abaAtualAbsenteismo);
    }

    if (lista.length === 0) {
        tbody.innerHTML = `<tr><td colspan="7" style="text-align:center; color:#9ca3af; padding: 20px;">Nenhum registro encontrado nesta categoria.</td></tr>`;
        return;
    }
    ...
    lista.forEach(item => {
        ...
        const nomeColaborador = item.rh_colaboradores ? item.rh_colaboradores.nome : '<span style="color:#ef4444;">Colaborador Removido</span>';
        const matricula = (item.rh_colaboradores && item.rh_colaboradores.cod_funcionario) ? String(item.rh_colaboradores.cod_funcionario).padStart(4, '0') : '';
        
        let badgeTipo = '';
        if(item.tipo_registro === 'ATESTADO') badgeTipo = '<span style="background:rgba(96,165,250,0.2); color:var(--ccol-blue-bright); padding:3px 8px; border-radius:12px; font-size:0.75rem; font-weight:bold;">Atestado</span>';
        else if(item.tipo_registro === 'FALTA') badgeTipo = '<span style="background:rgba(239,68,68,0.2); color:#ef4444; padding:3px 8px; border-radius:12px; font-size:0.75rem; font-weight:bold;">Falta/Ausência</span>';
        else badgeTipo = '<span style="background:rgba(61,220,132,0.2); color:var(--ccol-green-bright); padding:3px 8px; border-radius:12px; font-size:0.75rem; font-weight:bold;">Banco de Horas</span>';

        let detalhesHTML = '';
        if(item.tipo_registro === 'ATESTADO') {
            detalhesHTML = `<strong style="color:var(--ccol-blue-bright);"><i class="fas fa-user-md"></i> ${item.medico_nome || 'Médico não informado'}</strong><br><strong style="color:#f59e0b;">CID: ${item.cid || 'N/A'}</strong> - ${item.motivo || ''}`;
        } else {
            detalhesHTML = `<span>${item.motivo || 'Sem detalhes'}</span>`;
        }

        let periodoOuHoras = '';
        if(item.tipo_registro === 'ATESTADO') {
            periodoOuHoras = `${formatarData(item.data_inicio)} <i class="fas fa-arrow-right" style="font-size:0.7rem;"></i> <strong>${item.dias_afastamento || 1} dias</strong>`;
        } else if(item.tipo_registro === 'FALTA') {
            periodoOuHoras = `${formatarData(item.data_inicio)} (${item.quantidade_horas ? item.quantidade_horas + 'h' : 'Dia Inteiro'})`;
        } else {
            periodoOuHoras = `${formatarData(item.data_inicio)} (<strong style="color:var(--ccol-green-bright);">${item.quantidade_horas}h</strong>)`;
        }

        const btnAnexo = item.anexo_url ? `<a href="${item.anexo_url}" target="_blank" class="btn-primary-blue" style="padding: 4px 10px; font-size: 0.75rem; text-decoration: none; border-radius: 6px;"><i class="fas fa-paperclip"></i> Ver</a>` : '<span style="color:#64748b; font-size:0.8no;">-</span>';

        const tr = document.createElement('tr');
        tr.innerHTML = `
            <td style="font-size: 0.8rem; color: #94a3b8;">${dataLancamento}</td>
            <td style="text-align: left; font-weight: bold;"><span style="color:var(--ccol-blue-bright); font-size:0.8rem;">[${matricula}]</span> ${nomeColaborador}</td>
            <td>${badgeTipo}</td>
            <td style="text-align: left;">${detalhesHTML}</td>
            <td>${periodoOuHoras}</td>
            <td>${btnAnexo}</td>
            <td>
...
window.filtrarAbsenteismo = function() {
    ...
    window.listaAbsenteismoFiltrada = filtrados;
    window.renderizarTabelaAbsenteismo();
};
