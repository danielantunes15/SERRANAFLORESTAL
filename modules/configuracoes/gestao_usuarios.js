...
window.carregarFiliaisFormulario = async function() {
    ...
    if (window.currentUser && window.currentUser.role === 'SuperAdmin') {
        ...
        // Trava o usuário não-admin na filial dele
...
window.renderizarUsuarios = async function() {
    ...
    // Popula o formulário na inicialização da página
    ...
    try {
        ...
        // --- ISOLAMENTO DA LISTA DE USUÁRIOS POR FILIAL ---
        if (window.currentUser && window.currentUser.role !== 'SuperAdmin') {
...
window.adicionarUsuario = async function() {
    ...
    try {
        const novoUsuarioObj = { 
            username: nome, 
            senha_hash: "5994471abb01112afcc18159f6cc74b4f511b99806da59b3caf5a9c173cacfc5", // Hash da senha "12345"
            role: systemRole, 
